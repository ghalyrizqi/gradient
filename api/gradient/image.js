...
// api/gradient/image.ts
async function handler(req, res) {
  cors(res);
  if (req.method === "OPTIONS") return res.status(200).end();
  const p = parseUrl(req);
  const style = parseStyle(p.get("style"));
  const w = clamp(parseIntOr(p.get("w"), 1200), 100, 4e3);
  const h = clamp(parseIntOr(p.get("h"), 630), 100, 4e3);
  ...
  const stopTags = state.stops.map((s) => `<stop offset="${(s.pos * 100).toFixed(2)}%" stop-color="${s.color}"/>`).join("");
